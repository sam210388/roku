...
use App\Models\AnggaranRealisasi\Kegiatan;
...
class KegiatanController extends Controller
{
    ...
    public function kegiatan()
    {
        ...
        $lokasi = 'AnggaranRealisasi.kegiatan';
        $judul = 'Daftar Kegiatan';
        $crud = $this->_getGroceryCrudEnterprise();
        $tahunanggaran = session('tahunanggaran');
        $crud->where('tahunanggaran = '.$tahunanggaran);
        $crud->setTable('kegiatan');
        $crud->setSubject('Kegiatan', 'Kegiatan');
    ...
    function importkegiatan(){
        ...
        curl_setopt_array($curl, array(
            CURLOPT_URL => 'https://monsakti.kemenkeu.go.id/sitp-monsakti-omspan/webservice/API/KL002/refUraian/kegiatan',
        ...
        if ($diolah !== "" && $diolah !== "<b>Fatal error" ) {
            ...
            foreach ($hasilasli as $ITEM) {
                ...
                $jumlah = Kegiatan::where($where)->get()->count();
                if ($jumlah == 0){
                    $data = array(
                        'tahunanggaran' => $THANG,
                        'kode' => $KODE,
            ...
            return redirect()->to('anggaran/kegiatan')->with('status','Import Kegiatan Berhasil');
        }
    }
}
