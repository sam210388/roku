...
use App\Models\AnggaranRealisasi\Suboutput;
...
class SuboutputController extends Controller
{
    ...
    public function suboutput()
    {
        ...
        $lokasi = 'AnggaranRealisasi.suboutput';
        $judul = 'Daftar SubOutput';
        $crud = $this->_getGroceryCrudEnterprise();
        $tahunanggaran = session('tahunanggaran');
        $crud->where('tahunanggaran = '.$tahunanggaran);

        $crud->setTable('suboutput');
        $crud->setSubject('SubOutput', 'SubOutput');
    ...
    function importsuboutput(){
        ...
        curl_setopt_array($curl, array(
            CURLOPT_URL => 'https://monsakti.kemenkeu.go.id/sitp-monsakti-omspan/webservice/API/KL002/refUraian/suboutput',
        ...
        if ($diolah !== "" && $diolah !== "<b>Fatal error" ) {
            ...
            foreach ($hasilasli as $ITEM) {
                ...
                $jumlah = Suboutput::where($where)->get()->count();
                if ($jumlah == 0){
                    $data = array(
                        'tahunanggaran' => $THANG,
                        'kode' => $KODE,
                    ...
                    Suboutput::insert($data);
                }
            }
            return redirect()->to('anggaran/suboutput')->with('berhasil','Import SubOutput Berhasil');
        }
    }
}
